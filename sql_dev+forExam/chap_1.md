...
i) 인장신청시 QR코드 크기 조정 또는 위치 조정

두가지 모두 가능. 단 제한조건 있음.
크기 조정: 정사각형만 가능. 현재 사이즈는 90x90. => 하나만 받아서 변수로 넣어줌.
위치 조정: 세로로 상중하, 가로로 좌중우 설정 가능. 미세설정은 불가. 현재 설정은 오른쪽 상단(RT).

ii) 인장신청서 작성 및 결재요청 완료 후에, 기존에 작성했던 인장신청서 pop-up은 자동으로 close

popup이 종료되는 정확한 시점이 필요.
...
문의 내용 :
신청자 인장현황 화면에서 상기 관리번호 건이 안보인다고 하여 문의드립니다.
(신청자 정보: Workplace혁신팀 김소연 매니저님_사번:145000)
확인을 부탁드립니다. 감사합니다.
